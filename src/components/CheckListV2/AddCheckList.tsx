...
import { DEFAULT_PARAMS, getSelectedChecklistUserId } from "./types/checklist";
...
const AddChecklistModal: React.FC = () => {
  const navigate = useNavigate();
  const [selectedUserId] = useState<string>(getSelectedChecklistUserId());
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await createChecklist(submitData).unwrap();

      toast.success(`Checklist "${formData.title}" created successfully!`);
      navigate("/checklist");
    } catch (err: any) {
      console.error("Error saving checklist:", err);
      toast.error(
        err?.data?.message || `Failed to create checklist. Please try again.`,
      );
    }
  };
  ...
  const { data: assignee } = useGetAssignableUsersQuery({
    branch_id: apiParams.branch_id,
    zodu_id: apiParams.zodu_id,
  });
  const availableAssignees = (assignee?.Data || []).filter(
                          ...
                          return (
                            user && (
                              <Chip
                                key={user.id}
                                label={user.name}
                                onDelete={() => removeAssignee(user.id)}
          ...
          <Stack spacing={1.5} sx={{ mt: 1 }}>
            ...
            <Box>
              <input
                accept="image/*"
                style={{ display: "none" }}
                id="task-image-upload-modal"
                type="file"
                multiple
                onChange={handleImageUpload}
                disabled={isUploadingImages}
              />
              <label htmlFor="task-image-upload-modal">
                <Button
                  component="span"
                  variant="outlined"
