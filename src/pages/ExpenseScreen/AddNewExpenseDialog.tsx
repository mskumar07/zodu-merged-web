...
const theme = createTheme({
  ...
  components: {
    ...
    MuiOutlinedInput: {
      styleOverrides: {
        root: { borderRadius: 8, fontSize: 13, "&:hover .MuiOutlinedInput-notchedOutline": { borderColor: "#D21F3C" }, "&.Mui-focused .MuiOutlinedInput-notchedOutline": { borderColor: "#D21F3C", borderWidth: 2 } },
...
interface CatalogueItem {
  id: string; itemId: string; name: string; sku: string; category: string;
...
// ─── QtyCounter ───────────────────────────────────────────────
interface QtyCounterProps { value: number; onChange: (v: number) => void; disabled?: boolean; min?: number; size?: "compact" | "full"; }
function QtyCounter({ value, onChange, disabled = false, min = 0, size = "compact" }: QtyCounterProps) {
  ...
  return (
    <Box sx={{ display: "inline-flex", alignItems: "center", height: h, border: `1.5px solid ${border}`, borderRadius: "8px", overflow: "hidden", bgcolor: bg, transition: "border-color 0.15s, background 0.15s" }}>
      <Box component="button" onClick={dec} disabled={disabled || value <= min} sx={{ width: btnW, height: "100%", border: "none", borderRight: `1px solid ${border}`, bgcolor: "transparent", cursor: disabled || value <= min ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: disabled || value <= min ? "#CBD5E1" : btnColor, "&:hover:not(:disabled)": { bgcolor: "rgba(210,31,60,0.07)" }, p: 0, m: 0, flexShrink: 0 }}>
        <RemoveIcon sx={{ fontSize: fs + 1 }} />
      </Box>
      <Box component="input" type="number" step="1" value={value} disabled={disabled} onClick={e => e.stopPropagation()} onChange={e => { const v = Math.round(parseFloat(e.target.value)); if (!isNaN(v)) onChange(Math.max(min, v)); }} sx={{ width: numW, height: "100%", border: "none", outline: "none", textAlign: "center", fontSize: fs, fontWeight: 700, color: numColor, bgcolor: "transparent", fontFamily: "inherit", "&::-webkit-outer-spin-button, &::-webkit-inner-spin-button": { WebkitAppearance: "none" }, p: 0 }} />
      <Box component="button" onClick={inc} disabled={disabled} sx={{ width: btnW, height: "100%", border: "none", borderLeft: `1px solid ${border}`, bgcolor: "transparent", cursor: disabled ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: disabled ? "#CBD5E1" : btnColor, "&:hover:not(:disabled)": { bgcolor: "rgba(210,31,60,0.07)" }, p: 0, m: 0, flexShrink: 0 }}>
...
// ─── PriceInput ───────────────────────────────────────────────
interface PriceInputProps { value: number; onChange: (v: number) => void; disabled?: boolean; size?: "compact" | "full"; }
function PriceInput({ value, onChange, disabled = false, size = "compact" }: PriceInputProps) {
  const h = size === "compact" ? 28 : 32;
  const fs = size === "compact" ? 12 : 13;
  const w = size === "compact" ? 84 : 96;
  return (
    <Box sx={{ display: "inline-flex", alignItems: "center", height: h, border: "1.5px solid #E2E8F0", borderRadius: "8px", overflow: "hidden", bgcolor: disabled ? "#F8FAFC" : "#fff", "&:focus-within": { borderColor: "#D21F3C" }, "&:hover": { borderColor: disabled ? "#E2E8F0" : "#D21F3C" } }}>
      ...
      <Box component="input" type="number" value={value === 0 ? "" : value} placeholder="0.00" disabled={disabled} onClick={e => e.stopPropagation()} onChange={e => { const v = parseFloat(e.target.value); onChange(isNaN(v) ? 0 : Math.max(0, v)); }} sx={{ width: w, height: "100%", border: "none", outline: "none", textAlign: "right", fontSize: fs, fontWeight: 600, color: "#0F172A", bgcolor: "transparent", fontFamily: "inherit", px: "8px", "&::placeholder": { color: "#CBD5E1", fontWeight: 400 }, "&::-webkit-outer-spin-button, &::-webkit-inner-spin-button": { WebkitAppearance: "none" } }} />
    </Box>
  );
}
      ...
      {attachments.length > 0 && (
        <Box sx={{ mt: 1.5, display: "flex", flexDirection: "column", gap: 0.75 }}>
          {attachments.map(att => {
            ...
            return (
              <Box key={att.id} sx={{ display: "flex", alignItems: "center", gap: 1, p: "7px 10px", border: `1px solid ${att.uploading ? "#BFDBFE" : "#E5E7EB"}`, borderRadius: 2, bgcolor: att.uploading ? "#EFF6FF" : "#fff", "&:hover": { bgcolor: att.uploading ? "#EFF6FF" : "#FAFAFA" }, transition: "all 0.12s" }}>
                <Box sx={{ width: 36, height: 36, borderRadius: 1.5, overflow: "hidden", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", bgcolor: att.previewUrl ? "transparent" : cs.bg, border: "1px solid #E5E7EB", cursor: att.uploading ? "default" : "pointer", position: "relative" }} onClick={() => !att.uploading && setPreviewFile(att)}>
                  {att.uploading
                ...
                {!att.uploading && <Tooltip title="Remove"><IconButton size="small" onClick={() => onRemove(att.id)} sx={{ p: 0.5, color: "#9CA3AF", "&:hover": { color: "#EF4444", bgcolor: "#FEF2F2" }, borderRadius: 1 }}><CloseIcon sx={{ fontSize: 15 }} /></IconButton></Tooltip>}
              </Box>
            );
          })}
        </Box>
        ...
        <DialogContent sx={{ p: 0, bgcolor: "#0F172A", display: "flex", alignItems: "center", justifyContent: "center", minHeight: 320 }}>
          {previewFile?.previewUrl
            ...
            : <Box sx={{ py: 6, display: "flex", flexDirection: "column", alignItems: "center", gap: 2.5 }}>
                ...
                <Button variant="contained" size="small" startIcon={<DownloadOutlinedIcon sx={{ fontSize: 16 }} />} onClick={() => { if (previewFile) { const u = URL.createObjectURL(previewFile.file); const a = document.createElement("a"); a.href = u; a.download = previewFile.name; a.click(); URL.revokeObjectURL(u); } }} disableElevation sx={{ bgcolor: "#334155", color: "#E2E8F0", fontSize: 12, fontWeight: 700, px: 2.5, py: 0.8, borderRadius: 2, "&:hover": { bgcolor: "#475569" } }}>Download File</Button>
              </Box>}
        </DialogContent>
...
const itemSubtotal  = (i: ExpenseItem) => { const b = i.qty * i.unitPrice; return b + (b * i.taxPct) / 100; };
const itemTaxAmount = (i: ExpenseItem) => (i.qty * i.unitPrice * i.taxPct) / 100;
const catalogueToItem = (cat: CatalogueItem & { qty?: number }): ExpenseItem => ({
  id: `ei-${cat.id}-${Date.now()}-${Math.random()}`,
...
export default function AddNewExpenseDialog({ open, onClose, onSuccess, editExpenseId }: AddNewExpenseDialogProps) {
  ...
  const { data: vendorsResponse = [], isLoading: vendorsLoading } = useVendors("Expense");
  ...
  useEffect(() => {
    if (!isEditMode || !expenseDetail) return;
    const rawAtts: any[] = expenseDetail.attachment_url || [];
    setAttachments(
      rawAtts.map((file: any, i: number) => {
        if (typeof file === "string") {
          const name = file.split("/").pop() ?? `attachment-${i + 1}`;
          ...
          return { id: `existing-${i}-${file}`, url: file, name, size: 0, type, previewUrl: type.startsWith("image/") ? file : null };
        }
        const url = file.url ?? "";
        const name = file.filename ?? url.split("/").pop() ?? `attachment-${i + 1}`;
        const type = file.mimetype ?? "application/octet-stream";
        return { id: file.id ?? `existing-${i}`, url, name, size: file.size ?? 0, type, previewUrl: type.startsWith("image/") ? url : null };
      })
    );
    setForm({
  ...
  const paymentStatus = (): "paid" | "partial" | "pending" => {
    ...
    if (paid >= roundedTotal && roundedTotal > 0) return "paid";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(e?.response?.data?.message ?? e?.message ?? "Failed to save expense.");
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <Dialog open={open} onClose={handleDiscard} fullWidth maxWidth="lg" PaperProps={{ sx: { borderRadius: 3, maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 25px 60px rgba(15,23,42,0.2)" } }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            <Box>
              ...
              {isEditMode && editExpenseId && (
                <Typography sx={{ fontSize: 11, color: "#6B7280" }}>{editExpenseId}</Typography>
              )}
            </Box>
          </Box>
        ...
        <DialogContent sx={{ px: 3, py: 3, overflowY: "auto", flex: 1 }}>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            {/* Top fields: Supplier | Expense Date | Category */}
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr", lg: "1fr 1fr 1fr" }, gap: 2, mt: 1, alignItems: "start" }}>
              ...
              <Box>
                <Box sx={{ minHeight: 30, display: "flex", alignItems: "center", mb: 0.5 }}>
                  <FieldLabel sx={{ mb: 0, lineHeight: 1.2 }}>Expense Date</FieldLabel>
                </Box>
                <TextField type="date" size="small" fullWidth value={form.expenseDate} onChange={e => setField("expenseDate", e.target.value)} sx={inputSx} inputProps={{ style: { fontSize: 13 } }} />
              </Box>

              {/* Category */}
              <Box>
                <Box sx={{ minHeight: 30, display: "flex", alignItems: "center", mb: 0.5 }}>
                  <FieldLabel sx={{ mb: 0, lineHeight: 1.2 }}>Select Category *</FieldLabel>
                </Box>
                <FormControl size="small" fullWidth sx={inputSx}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 1fr 1fr" }, gap: 3, alignItems: "start" }}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
                <Box>
                  <FieldLabel>Expense Notes</FieldLabel>
              ...
              <Paper elevation={0} sx={{ bgcolor: "#F8FAFC", border: "1px solid #E5E7EB", borderRadius: 3, p: 2.5, display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                <Box sx={{ pt: 1.5, borderTop: "1px solid #E5E7EB", display: "flex", flexDirection: "column", gap: 1 }}>
                  ...
                  <TextField type="date" size="small" fullWidth value={form.dueDate ?? ""} onChange={e => setField("dueDate", e.target.value)} sx={inputSx} inputProps={{ style: { fontSize: 13 } }} />
                  {form.dueDate && (
