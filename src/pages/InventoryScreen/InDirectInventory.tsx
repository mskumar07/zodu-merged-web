...
import { useGetInventoryListQuery, useAddIndirectInventoryMutation } from "@store/services/inventoryApi";
...
import IndirectInventoryDrawer from "@components/InventoryTable/InventoryDrawer"; //Z-T87



const InDirectInventoryScreen: React.FC = () => {
  const [selectedItem, setSelectedItem] = useState<MenuItem | null>(null);
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [viewModalOpen, setViewModalOpen] = useState(false);
  const [searchValue, setSearchValue] = useState("");
  // Replace this with your actual branch ID
  const branchId = useSelector(BranchId);
  // ✅ Fetch inventory data from API with inventoryType "indirect"
  const { data: inventoryData, isLoading: isInventoryLoading, isError: isInventoryError } =
    useGetInventoryListQuery({ branchId: branchId, inventoryType: "indirect" });

  const [addIndirectInventory, { isLoading }] = useAddIndirectInventoryMutation(); //Z-T87
  ...
  const handleSubmit = async (values, { resetForm }) => {
    try {
      const response = await addIndirectInventory(values).unwrap();
  ...
  return (
    <Box>
      ...
      <Box sx={{ backgroundColor: "white", padding: 2, borderRadius: 2, minHeight: "300px", display: "flex", alignItems: "center", justifyContent: isInventoryLoading ? "center" : "flex-start" }}>
        {isInventoryLoading ? (
      ...
      /> */}
      {/*Z-T87 */}
      <IndirectInventoryDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} handleSubmit={handleSubmit} />
    </Box>
  );
};

export default InDirectInventoryScreen;
