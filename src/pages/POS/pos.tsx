...
//   useEffect(() => {
//     if (!catalogueLoading && zone === "SEARCH" && searchFocus === "CODE" && document.activeElement === codeRef.current)
//       setShowSuggestions(true);
//   }, [catalogueLoading]);
...
//     setItems(prev => prev.map(i => i.code === code
//       ? { ...i, editingPrice: true, editingQty: false, priceDraft: String(i.unitPrice) }
...
//   // Discount is applied on subtotal (pre-tax); GST is then charged on the discounted subtotal
...
//   // GST rates per item, applied proportionally on the discounted subtotal
//   const gstAmount       = useMemo(() => {
//     if (subtotal === 0) return 0;
//     return items.reduce((s, i) => {
//       const itemSubtotal = i.qty * i.unitPrice;
//       // Each item's share of the discount
//       const itemDiscount = discountAmt * (itemSubtotal / subtotal);
//       return s + Math.max(0, itemSubtotal - itemDiscount) * i.gstPct / 100;
//     }, 0);
...
//                     onFocus={() => { setZone("SEARCH"); setSearchFocus("CODE"); if (!catalogueLoading) setShowSuggestions(true); }}
...
//                             <Box sx={{ minWidth: 0 }}>
//                               <Typography sx={{ fontSize: 12, fontWeight: 800, color: "#1F2937" }}>{customerLabel(c)}</Typography>
//                               <Typography sx={{ fontSize: 10, color: "#9CA3AF" }}>{primaryMobile(c)}{c.city ? ` • ${c.city}` : ""}</Typography>
//                             </Box>
...
// ─── Types ────────────────────────────────────────────────────
interface LineItem {
  code: string; description: string; qty: number; unitPrice: number; sellPrice: number;
                        ...
                        <Box sx={{ maxHeight: 300, overflowY: "auto", bgcolor: "#fff" }}>
                          {suggestions.map((p, idx) => (
                            <Box key={p.item_id} onMouseDown={() => selectSuggestion(p)}
                              ...
                              <Box sx={{ textAlign: "right", flexShrink: 0 }}>
                                <Typography sx={{ fontSize: 13, fontWeight: 700, color: idx === suggestionIdx ? "#1D4ED8" : "#111827" }}>₹{Number(p.sell_price).toLocaleString("en-IN")}</Typography>
                                <Typography sx={{ fontSize: 9, color: p.tax_inclusive ? "#D97706" : "#9CA3AF", fontWeight: p.tax_inclusive ? 700 : 400, lineHeight: 1 }}>
                                  {p.tax_inclusive ? "GST incl." : "+"+(Number(p.gst_tax)||0)+"%GST"} · {p.unit || "NOS"}
                                </Typography>
                              </Box>
                            </Box>
                          ))}
