...
interface AttachmentFile {
  id: string; file: File; name: string; size: number; type: string; previewUrl: string | null;
}
...
function AttachmentsSection({ attachments, onAdd, onRemove }: AttachmentsSectionProps) {
  ...
  const processFiles = (fileList: FileList | null) => {
    ...
    onAdd(Array.from(fileList).map(file => {
      const t = resolveType(file);
      return { id: `att-${Date.now()}-${Math.random()}`, file, name: file.name, size: file.size, type: t, previewUrl: t.startsWith("image/") ? URL.createObjectURL(file) : null };
    }));
  };
      ...
      {attachments.length > 0 && (
        <Box sx={{ mt: 1.5, display: "flex", flexDirection: "column", gap: 0.75 }}>
          {attachments.map(att => {
            ...
            return (
              <Box key={att.id} sx={{ display: "flex", alignItems: "center", gap: 1, p: "7px 10px", border: "1px solid #E5E7EB", borderRadius: 2, bgcolor: "#fff", "&:hover": { bgcolor: "#FAFAFA", borderColor: "#D1D5DB" }, "&:hover .att-a": { opacity: 1 }, transition: "all 0.12s" }}>
                <Box sx={{ width: 36, height: 36, borderRadius: 1.5, overflow: "hidden", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", bgcolor: att.previewUrl ? "transparent" : cs.bg, border: "1px solid #E5E7EB", cursor: "pointer", position: "relative", "&:hover .tov": { opacity: 1 } }} onClick={() => setPreviewFile(att)}>
                  {att.previewUrl ? (<><img src={att.previewUrl} alt={att.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /><Box className="tov" sx={{ position: "absolute", inset: 0, bgcolor: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", opacity: 0, transition: "opacity 0.15s", borderRadius: 1.5 }}><ZoomInIcon sx={{ fontSize: 16, color: "#fff" }} /></Box></>) : <Box sx={{ display: "flex" }}>{getFileIcon(att.type)}</Box>}
                </Box>
                <Box sx={{ flex: 1, minWidth: 0, cursor: "pointer" }} onClick={() => setPreviewFile(att)}>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, mt: 0.2 }}>
                    <Chip label={getFileExt(att.name)} size="small" sx={{ fontSize: 9, fontWeight: 800, height: 16, px: 0.3, bgcolor: cs.bg, color: cs.color, border: "none" }} />
                    <Typography sx={{ fontSize: 10, color: "#9CA3AF" }}>{formatBytes(att.size)}</Typography>
                  </Box>
                </Box>
                <Box className="att-a" sx={{ display: "flex", gap: 0.5, opacity: 0, transition: "opacity 0.15s", flexShrink: 0 }}>
...
export default function AddNewPurchaseDialog({
  ...
  const handleSave = async () => {
    ...
    : null,      items: form.items.map((item): PurchaseItemPayload => {
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 1fr 1fr" }, gap: 3, alignItems: "start" }}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2.5 }}>
                ...
                <Box>
                  ...
                  <AttachmentsSection attachments={attachments} onAdd={handleAddAtts} onRemove={handleRemoveAtt} />
                </Box>
              </Box>
