...
import {
  useDatewiseExpenseSummary,
  useDatewiseExpenseBreakdown,
  type DatewiseExpenseBreakdownRow,
...
const DatewiseExpenseReport = () => {
  ...
  const {
    ...
    hasNextPage,
    fetchNextPage,
  } = useDatewiseExpenseBreakdown({ ...params, limit: 15 });

  const rows: DatewiseExpenseBreakdownRow[] = useMemo(
  ...
  const columns = useMemo<ColumnDef<DatewiseExpenseBreakdownRow>[]>(
    () => [
      {
        key: "expenseDate",
        label: "DATE",
      ...
      },
      {
        ...
        minWidth: 130,
        render: (row) => (
          <Typography sx={{ fontSize: "0.8rem", fontWeight: "bold", color: "#D92D20", whiteSpace: "nowrap" }}>
            {fmt(row.paidAmount)}
          </Typography>
        ),
      },
        ...
        <Box sx={{ width: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <Paper sx={{ borderRadius: 1, border: "1px solid #eee", display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
            ...
            <Box sx={{ flex: 1, minHeight: 0 }}>
              <DataTable<DatewiseExpenseBreakdownRow>
                columns={columns}
                rows={rows}
                rowKey={(row) => row.expenseDate}
                isLoading={breakdownLoading}
                skeletonRows={8}
                ...
                maxHeight="100%"
                emptyMessage="No expense data found for the selected date range"
              />
            </Box>
