...
interface DeleteCompanyDialogProps {
  open: boolean;
  company: CompanyWithBranches | null;
  loading?: boolean;
  onClose: () => void;
  onConfirm: () => void;
}
      ...
      <Dialog open={open} onClose={loading ? undefined : onClose} maxWidth="xs" fullWidth>
        <Box sx={{ position: "relative", px: 3.5, pt: 3.5, pb: 3 }}>
          ...
          <Typography sx={{ fontSize: 14, color: "#6B7280", lineHeight: 1.6, mb: 1.5 }}>
            ...
            {branchCount > 0 ? (
              <>
                {" "}and all {branchCount} of its branch{branchCount === 1 ? "" : "es"}
              </>
            ) : null}
            . This removes all sales, customers, staff, payroll, and checklist
            data tied to this company across every service.
          </Typography>
          ...
          >
            <Typography sx={{ fontSize: 12.5, fontWeight: 700, color: "#af101a" }}>
              This action cannot be undone. Deleted company data cannot be restored.
            </Typography>
          </Box>
