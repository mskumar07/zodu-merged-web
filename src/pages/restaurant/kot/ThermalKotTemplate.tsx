...
const PAPER: Record<ThermalPaperSize, { widthPx: number; fs: number; padding: string }> = {
  "2": { widthPx: 181, fs: 13, padding: "8px 2px 20px 2px" },
  "3": { widthPx: 272, fs: 14, padding: "10px 4px 24px 4px" },
...
function Row({ label, value, fontSize, strong }: { label: string; value: string; fontSize: number; strong?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, fontSize, lineHeight: 1.4, marginBottom: 2 }}>
      ...
      <span style={{ fontWeight: 600, textAlign: "right", whiteSpace: "nowrap" }}>{value}</span>
    </div>
  );
}
    ...
    return (
      ...
      >
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: fs }}>
          <thead>
            <tr>
              <th style={{ textAlign: "left", fontWeight: 600, fontSize: small, paddingBottom: 3, borderBottom: "1.5px solid #000" }}>Item</th>
              <th style={{ textAlign: "right", fontWeight: 600, fontSize: small, paddingBottom: 3, paddingLeft: 8, borderBottom: "1.5px solid #000", whiteSpace: "nowrap" }}>Qty</th>
            </tr>
          </thead>
          <tbody>
            {data.items.map((it, i) => (
